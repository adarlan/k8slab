...
terraform-apply() {
    ...
    terraform -chdir=$1 apply -var-file=.tfvars $AUTO_APPROVE
}

terraform-destroy() {
    terraform -chdir=$1 init
    terraform -chdir=$1 destroy -var-file=.tfvars $AUTO_APPROVE
}

terraform-plan-apply() {
    terraform -chdir=$1 init
    terraform -chdir=$1 plan -var-file=.tfvars -out apply.tfplan
}

terraform-plan-destroy() {
    terraform -chdir=$1 init
    terraform -chdir=$1 plan -destroy -var-file=.tfvars -out destroy.tfplan
}

terraform-apply-with-plan() {
    terraform -chdir=$1 apply apply.tfplan
}
